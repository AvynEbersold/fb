...
/* THIS SCRIPT IS FOR RUNNING THE GAME AND ALL OF ITS COMPONENTS. DEFAULT WEBSITE BEHAVIOR FOR MY GAMES
...
let game = {
  ...
  distanceTraveled: 0,
  scrollSpeed: 2,
  groundY: 0.85, //HOW FAR DOWN THE GROUND IS ON CANVAS
  gameDarkness: 0,
...
function displayForeground() {
  ...
  if (game.gameOver == false) {
    ...
    c.drawImage(playerImage, 0 - ((game.width / player.size) / 2), 0 - ((game.height / player.size) / 2), game.width / player.size, game.height / player.size);
    c.restore();
